...
class Model(object):
    ...
    def update_pi(self, R):
        ...
        sum_to_k = R.shape[1]
        numerator = np.sum(R, axis=0) + a_k - 1
        denominator = np.sum(R) + sum_to_k*(a_k - 1)
    ...
    def update_theta(self, X, R):
        ...
        denominator = np.sum(R, axis=0).reshape(-1,1)+b+a-2
        return np.divide(numerator,denominator)
    ...
    def posterior_predictive_means(self, X, M):
        ...
        ######################## Your code here #########################
        my_pos = self.compute_posterior(X,M)
        return np.dot(my_pos,self.params.theta)
...
if __name__ == '__main__':
    ...
    em_model = train_with_em()
    lable_model = train_from_labels()
    print_log_probs_by_digit_class(em_model)
    print("---------------------------")
    print_log_probs_by_digit_class(lable_model)
